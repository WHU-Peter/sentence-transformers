...
class CLIPModel(nn.Module):
    def __init__(self, tokenizer_path: str, model_name: str = "ViT-B/32"):
        ...
        self.model, self.preprocess = load(model_name, device=device)
    ...
    def forward(self, features):
        ...
        if len(features['text_features']) > 0:
            text_features = self.model.encode_text(features['text_features'])
        if len(features['image_features']) > 0:
            image_features = self.model.encode_image(features['image_features'])

        sentence_embedding = []
        text_features = iter(text_features)
        image_features = iter(image_features)
    ...
    def save(self, output_path: str):
        ...
        tokenizer_name = self.tokenizer.save(output_path)
    ...
    def load(input_path: str):
        ...
        model_path = os.path.join(input_path, config['model_name'])
        tokenizer_path = os.path.join(input_path, config['tokenizer_name'])
        return CLIPModel(tokenizer_path=tokenizer_path, model_name=model_path)
